...
export const TransactionBankReport3 = props => {
	...
	const getDataTransaction = async () => {
		...
		if (resItem) {
		    ...
		    await _.map(resItem, async (e, index) => {
				...
				if (startTime <= moment(e.transactionDateandTime).unix() && moment(e.transactionDateandTime).unix() <= endTime) {
					...
					// if(!_.isUndefined(e.bankPay) && e.bankPay == 'KBANK' && e.payeeName == ''){
					// 	console.log('e.bankPay', index) 
					// 	const refMachine = _.replace(e.billPaymentRef1, "UP", "");
					...
					// 		await Database.UpdateDataTransaction(moment(startDate).format('YYYY-MM'),index,{updateMachineTime:true})
					// 	}
					// 	// console.log('snapshot',k[0], data.branch)
					// }

					if ( !_.isUndefined(dataM[e.keyMachine]) && !_.isUndefined(e.bankPay) && e.bankPay == 'KBANK') {
						...
						console.log("🚀 ~ await_.map ~ wallet:", permission.length)
						console.log('typeChannel', typeChannel)
